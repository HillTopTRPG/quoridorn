  ...
  <WindowFrame titleText="BGM編集画面" display-property="private.display.editBGMWindow" align="right-bottom" fixSize="300, 350" @open="initWindow">
    <div class="contents">
      <fieldset>
        <legend>読込</legend>
        <!-- URL -->
        <label class="url"><span>URL</span><input type="text" v-model="url"></label>
      </fieldset>
      <fieldset>
        <legend>表示</legend>
        <div class="firstWide">
      ...
      <fieldset>
        ...
        <div>
          ...
          <VolumeComponent
            ...
            ref="volumeComponent"/>
          <!-- プレビュー -->
          <button class="preview">プレビュー</button>
        </div>
        <div>
          <!-- </label> -->
          <!-- 再生時間 -->
      ...
      <fieldset>
        ...
        <div class="lastWide">
          ...
          <label
            ...
            <input :placeholder="chatLinkage === 1 ? '' : 'Javascriptの正規表現'" type="text" v-model="search"></label>
        </div>
      </fieldset>
    </div>
  </WindowFrame>
...
export default {
  ...
  methods: {
    ...
    initWindow () {
      ...
      this.volume = bgmObj.volume
      this.chatLinkage = bgmObj.chatLinkage
      this.search = bgmObj.chatLinkageSearch
    },
    change (param) {
